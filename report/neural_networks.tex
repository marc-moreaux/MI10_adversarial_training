...
\label{sec:neural_networks}
	...
	\label{sec:back_propagation}
		...
		\subsubsection{Step-by-step}
			...
			The \textbf{second} step is to back-propagate the cost to the other layers $l$. We will, more than ever here, take advantage of the chain rule.
			\begin{equation}
				\begin{split}
					\boldsymbol{\delta}^l
					&= \frac{\partial C}{\partial \boldsymbol{z}^l } \\
		...
		\subsubsection{Equations applied to our simple example}
			...
			\begin{itemize}
				\item The derivative of the cost $C$ with respect to the predictions $p_j$:
					...
					\end{equation}
				\item The derivative of a neuron's output $a^l_j$ with respect to its inputs $z^l_j$ (or $\boldsymbol{a}^l$ wrt. $\boldsymbol{z}^l$).
					\vskip 1em
					\begin{tabular}{c|c}
						\parbox{14em}{
							\begin{equation}
							...
							\begin{equation}
								\begin{split}
									...
									&= \boldsymbol{z}^l \circ (1-\boldsymbol{z}^l)
								\end{split}
							\end{equation}
						}
					\end{tabular}
